        ...
        foreach ($this->_db as $name => $adapter) {
            if ($profiles = $adapter->getProfiler()->getQueryProfiles()) {
                ...
                foreach ($profiles as $profile) {
                    ...
                    if ($paramCount) {
                        $queries .= htmlspecialchars(preg_replace(array_fill(0,
                                    $paramCount, '/\?/'), $params,
                                $profile->getQuery(), 1));
                    } else {
                    ...
                    if ($this->_explain && $supportedAdapter) {
                        ...
                        foreach ($adapter->fetchAll('EXPLAIN ' . $profile->getQuery()) as
                                $explain) {
                            $queries .= "<div style='padding-bottom:0.5em'>";
                            $explainData = array(
                                'Type' => $explain['select_type'] . ', ' . $explain['type'],
                                'Table' => $explain['table'],
                                'Possible keys' => str_replace(',', ', ',
